#!/bin/bash
# Compila driver_verifica_prereparto (v3-cuda-optimizacion/fase3): verifica
# en CPU pura, contra la denssumapaso real, que el histograma pre-reparto
# pesado por nsons coincide bit a bit con el histograma post-reparto real
# -- antes de escribir k_fase_h en el pipeline GPU. Mismo file list que
# compilar_diagnostico.sh, sustituyendo el programa principal.
set -e
cd "$(dirname "$0")"
...
modlegendre.f pw_heocs.f kpcoef.f driver_verifica_prereparto.f90"

nvfortran -cuda -Kieee -Mnofma -gpu=lineinfo -c $FILES
nvfortran -cuda -Kieee -Mnofma -gpu=lineinfo -o driver_verifica_prereparto $(echo $FILES | sed -E 's/\.(f90|f|cuf)\b/.o/g') -llapack -lblas
echo "OK: driver_verifica_prereparto"

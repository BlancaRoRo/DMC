#!/bin/bash
# Compila gpu-mejorado-instrumentado/qmccluster_pipeline -- copia local,
# no toca hibrido_instrumentado/. Mismo FILES que
# compilar_pipeline.sh.
set -e
cd "$(dirname "$0")/gpu-mejorado-instrumentado"
...
nvfortran -cuda -Kieee -Mnofma -gpu=lineinfo -o qmccluster_pipeline $(echo $FILES | sed -E 's/\.(f90|f|cuf)\b/.o/g') -llapack -lblas -lnvhpcwrapnvtx
echo "OK: qmccluster_pipeline (gpu-mejorado-instrumentado)"

...
! (arriba) no se toca.
 subroutine dmc_gpu_pipeline
  ...
  ! ---- v3-cuda-optimizacion/tiempos-metodos-dmc: mismo cronometraje
  ! metodo a metodo que la copia CPU instrumentada (cpu-original-
  ! instrumentado/mmontecarlo.f90), para comparar directamente contra
  ! ella -- ver tiempos-metodos-dmc.md. system_clock puro, en paralelo
  ! a la instrumentacion tiempos_tic/tiempos_toc que ya tenia esta
  ! subrutina (t_total7 etc., mtiempos.f90, sin tocar). Nota: en esta
  ! via, "denssumapaso" (CPU, por paso) ya no existe como tal -- su
  ! trabajo se fusiono dentro de pasodmc_gpu_pipeline (k_fase_h, GPU) y
  ! solo queda un volcado por BLOQUE (vuelca_histogramas_gpu), no por
  ! paso -- se etiqueta por separado para no mezclar cosas distintas. ----
  integer(kind=8) :: cc0,cc1,tasa_reloj
  ...
  real(kind=r8) :: t_resetea_hist=0
  real(kind=r8) :: t_pasodmc_gpu=0, t_dmcsumapaso_m=0, t_difussumapaso_m=0
  real(kind=r8) :: t_vuelca_hist=0
  ...
  integer(kind=4) :: istat_nvtx

   call system_clock(count_rate=tasa_reloj)
   call system_clock(cc0)

   call iniimagina(nombre,natom,dtau,hb2x,brot)
   call system_clock(cc1); t_iniimagina=t_iniimagina+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1

   call dmcnumprop(dmcnvalblo,dmcnvaltot)
   call system_clock(cc1); t_dmcnumprop=t_dmcnumprop+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
   call densgethis(ndens1,nhis1,ndens2,nhis2)
   call system_clock(cc1); t_densgethis=t_densgethis+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
   call difusgethis(npol,ndt)
   call system_clock(cc1); t_difusgethis=t_difusgethis+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
   ndifus=min(ndt,npasos)
   ...
   ndifus=min(ndt,npasosblo)
   if(soydire) call dmcescrini(equilbr)
   call system_clock(cc1); t_dmcescrini=t_dmcescrini+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1

   call dmcceroini
   call system_clock(cc1); t_dmcceroini=t_dmcceroini+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
   call densceroini
   call system_clock(cc1); t_densceroini=t_densceroini+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
   call difusceroini
   call system_clock(cc1); t_difusceroini=t_difusceroini+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
   call tiempos_reset
   call tiempos_tic(tt0run)
   do iblock=1,nblockeq+nblock
     if(iblock.eq.nblockeq+1) then
     ...
     call dmcceroblo
     call system_clock(cc1); t_dmcceroblo=t_dmcceroblo+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
     call densceroblo
     call system_clock(cc1); t_densceroblo=t_densceroblo+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
     istat_nvtx = nvtxRangePush("resetea_histogramas_gpu")
     call resetea_histogramas_gpu
     istat_nvtx = nvtxRangePop()
     call system_clock(cc1); t_resetea_hist=t_resetea_hist+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
     denb_local=0.0_r8
     call difusceroblo
     call system_clock(cc1); t_difusceroblo=t_difusceroblo+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
     call difusfijaorigen(nwpaso,wsim)
     call system_clock(cc1); t_difusfijaorigen=t_difusfijaorigen+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
     do ipaso=1,npasosblo
       istat_nvtx = nvtxRangePush("pasodmc_gpu_pipeline")
       call pasodmc_gpu_pipeline(nwpaso,egrow,wsim)
       istat_nvtx = nvtxRangePop()
       call system_clock(cc1); t_pasodmc_gpu=t_pasodmc_gpu+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
       call tiempos_tic(tt0)
       call dmcsumapaso(nwpaso,egrow,wsim)
       t_dmcsumapaso=t_dmcsumapaso+tiempos_toc(tt0)
       call system_clock(cc1); t_dmcsumapaso_m=t_dmcsumapaso_m+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
       ...
       call system_clock(cc1); t_difussumapaso_m=t_difussumapaso_m+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
     enddo
     call tiempos_tic(tt0)
     call vuelca_histogramas_gpu
     call densputdenb(denb_local)
     t_denssumapaso=t_denssumapaso+tiempos_toc(tt0)
     call system_clock(cc1); t_vuelca_hist=t_vuelca_hist+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
     call dmcsumablo
     call system_clock(cc1); t_dmcsumablo=t_dmcsumablo+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
     call denssumablo
     call system_clock(cc1); t_denssumablo=t_denssumablo+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
     call difussumablo(ndifus)
     call system_clock(cc1); t_difussumablo=t_difussumablo+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
     call dmcescrblopar(dmcnvalblo)
     call system_clock(cc1); t_dmcescrblopar=t_dmcescrblopar+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
   enddo
   ...
   call dmcsumaproc(dmcnvaltot)
   call system_clock(cc1); t_dmcsumaproc=t_dmcsumaproc+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
   call denssumaproc(ndens1,nhis1,ndens2,nhis2)
   call system_clock(cc1); t_denssumaproc=t_denssumaproc+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
   call difussumaproc(npol,ndifus)
   call system_clock(cc1); t_difussumaproc=t_difussumaproc+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1

   if(soydire) then
     call dmcsumafin
     call system_clock(cc1); t_dmcsumafin=t_dmcsumafin+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
     call dmcescrfin(dtau,cmtok,k2mev_bh,opot)
     call system_clock(cc1); t_dmcescrfin=t_dmcescrfin+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
     call denssumafin
     call system_clock(cc1); t_denssumafin=t_denssumafin+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
     call densescrfin
     call system_clock(cc1); t_densescrfin=t_densescrfin+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
     call difusfin(ndifus)
     call system_clock(cc1); t_difusfin=t_difusfin+real(cc1-cc0,r8)/tasa_reloj; cc0=cc1
   endif

   t_total_dmc = t_iniimagina+t_dmcnumprop+t_densgethis+t_difusgethis+t_dmcescrini+ &
     t_dmcceroini+t_densceroini+t_difusceroini+t_dmcceroblo+t_densceroblo+t_resetea_hist+ &
     t_difusceroblo+t_difusfijaorigen+t_pasodmc_gpu+t_dmcsumapaso_m+t_difussumapaso_m+ &
     t_vuelca_hist+t_dmcsumablo+t_denssumablo+t_difussumablo+t_dmcescrblopar+t_dmcsumaproc+ &
   ...
   write(uverif,'(a,f12.4,a,f7.3,a)') 'resetea_histogramas_gpu (nuevo)', t_resetea_hist, ' s  (', 100*t_resetea_hist/t_total_dmc, '%)'
   ...
   write(uverif,'(a,f12.4,a,f7.3,a)') 'pasodmc_gpu_pipeline (=pasodmc)', t_pasodmc_gpu, ' s  (', 100*t_pasodmc_gpu/t_total_dmc, '%)'
   write(uverif,'(a,f12.4,a,f7.3,a)') 'dmcsumapaso       ', t_dmcsumapaso_m, ' s  (', 100*t_dmcsumapaso_m/t_total_dmc, '%)'
   write(uverif,'(a,f12.4,a,f7.3,a)') 'difussumapaso     ', t_difussumapaso_m,' s  (', 100*t_difussumapaso_m/t_total_dmc,'%)'
   write(uverif,'(a,f12.4,a,f7.3,a)') 'vuelca_histogramas_gpu (=denssumapaso, por BLOQUE)', t_vuelca_hist, ' s  (', 100*t_vuelca_hist/t_total_dmc, '%)'

...
: > "$HERE/wall_times_op7_mejorado.log"
...
"
  echo "=== op7_${nw}walkers (mejorado) ===" | tee -a "$HERE/wall_times_op7_mejorado.log"
  cd "$GPU_DIR"
  t0=$(date +%s.%N)
  timeout 600 ./qmccluster_pipeline < in.mcv > "$HERE/op7_mejorado_${nw}walkers.log" 2>&1
  t1=$(date +%s.%N)
  dt=$(echo "$t1 - $t0" | bc)
  echo "  wall=${dt}s" | tee -a "$HERE/wall_times_op7_mejorado.log"
  cd "$HERE"
done

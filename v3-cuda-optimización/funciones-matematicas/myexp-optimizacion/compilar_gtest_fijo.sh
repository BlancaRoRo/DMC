#!/bin/bash
# Compila gpu-gtest-fijo/qmccluster_pipeline -- copia aislada donde
# GTEST de He_dihydrogen ya no es un argumento en tiempo de ejecucion,
# sino un PARAMETER=.false. fijo en compilacion (el unico call site
# real, potenbh, siempre pasaba .false.). Con GTEST conocido en
# compilacion, el compilador puede eliminar las 3 ramas IF(GTEST) por
# completo. No toca hibrido_instrumentado real.
set -e
cd "$(dirname "$0")/gpu-gtest-fijo"
...
echo "OK: qmccluster_pipeline (gpu-gtest-fijo)"

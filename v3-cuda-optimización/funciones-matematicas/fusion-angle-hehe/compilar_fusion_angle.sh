#!/bin/bash
# Compila gpu-fusion-angle/qmccluster_pipeline -- produccion actual pero
# con la llamada a angle() en He_dihydrogen sustituida por myacos(fi)
# directo, evitando recalcular sprod/norm1/norm2 (ya calculados como
# ror/rnorm/onorm justo antes). Ver fusion-angle-hehe.md.
set -e
cd "$(dirname "$0")/gpu-fusion-angle"
...
echo "OK: qmccluster_pipeline (gpu-fusion-angle)"

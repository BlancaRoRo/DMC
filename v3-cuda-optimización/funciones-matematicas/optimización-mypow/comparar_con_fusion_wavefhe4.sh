...
RESULT_DIR="/home/blanca/Documentos/Universidad/TFG-CUDA/Repostorio/DMC/v3-cuda-optimización/optimización-mypow/resultados_varios_walkers_con_fusion_wavefhe4"
mkdir -p "$RESULT_DIR"
...
"
  t0=$(date +%s.%N)
  (cd "$REF" && timeout 600 ./qmccluster_pipeline < in.mcv > "$RESULT_DIR/referencia2_${nw}w.log" 2>&1)
  t1=$(date +%s.%N)
  echo "  wall=$(echo "$t1 - $t0" | bc)s"
  cp "$REF/tiempos_opcion7.dat" "$RESULT_DIR/referencia2_${nw}w_tiempos_opcion7.dat"
...
"
  t0=$(date +%s.%N)
  (cd "$OPT" && timeout 600 ./qmccluster_pipeline < in.mcv > "$RESULT_DIR/optimizado2_${nw}w.log" 2>&1)
  t1=$(date +%s.%N)
  echo "  wall=$(echo "$t1 - $t0" | bc)s"
  cp "$OPT/tiempos_opcion7.dat" "$RESULT_DIR/optimizado2_${nw}w_tiempos_opcion7.dat"
done

#!/bin/bash
# Compila cambio-**/hibrido_instrumentado/qmccluster_pipeline -- copia
# aislada donde mypow/mypow_log/mypow_desde_log (glibc_pow.cuf) usan el
# operador ** nativo de nvfortran en vez del puerto bit-exacto de
# __pow. Ningun otro fichero se toca -- misma lista de compilacion que
# compilar_pipeline.sh (v2-cuda-integracion/test-tiempos). No afecta a
# hibrido_instrumentado real ni a gpu-duhe4x-optimizado.
set -e
cd "$(dirname "$0")/hibrido_instrumentado"
...
echo "OK: qmccluster_pipeline (cambio-**)"

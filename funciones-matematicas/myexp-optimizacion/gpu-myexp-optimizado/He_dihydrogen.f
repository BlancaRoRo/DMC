...
C  v3-cuda-optimizacion/myexp-optimizacion: FN1...DFN2 llamaban a
C  F0(XDUMM)...F6(XDUMM)/DF0(XDUMM)...DF6(XDUMM), y CADA UNA de esas
C  volvia a llamar a F00(XDUMM)=myexp(-XDUMM) por su cuenta (7 veces
C  para FN1/DFN1, 3 para FN2/DFN2, mismo XDUMM siempre). Aqui se
C  despliegan las formulas de F0...F6/DF0...DF6 directamente (F0...F6
C  y DF0...DF6 NO se tocan, se dejan intactas sin usar como
C  referencia) y se saca F00(XDUMM) como factor comun, calculado UNA
C  vez en F00X. Cada termino es la misma operacion que antes (F00X en
C  vez de F00(XDUMM)/DF0(XDUMM), que vale lo mismo), en el mismo orden
C  de suma -- bit a bit identico, verificado en myexp-optimizacion.md.
...
C  v3-cuda-optimizacion/myexp-optimizacion: fn2_rh1/fn2_rh2/fn2_r0 =
C  FN2(btheta*rh1)/FN2(btheta*rh2)/FN2(btheta*r0), calculados UNA vez
C  cada uno y reutilizados en Ex0/Ey0/Ez0 (antes cada uno llamaba a
C  FN2 con el mismo argumento, 3 veces). Ver myexp-optimizacion.md.
      DOUBLE PRECISION fn2_rh1,fn2_rh2,fn2_r0
      LOGICAL USE_TREESUM
...
C  comparten el mismo GTEST, no hay desincronizacion posible).
     ...
     &    (X(3*(J1-1)+3))**2
       ...
       Ex0=q*fn2_rh1*(X(3*(J1-1)+1)-r_dih(1,1))/rh1**3
       Ex0=Ex0+q*fn2_rh2*(X(3*(J1-1)+1)-r_dih(1,2))/rh2**3
       Ex0=Ex0-q0*fn2_r0*(X(3*(J1-1)+1))/r0**3

       Ey0=q*fn2_rh1*(X(3*(J1-1)+2)-r_dih(2,1))/rh1**3
       Ey0=Ey0+q*fn2_rh2*(X(3*(J1-1)+2)-r_dih(2,2))/rh2**3
       Ey0=Ey0-q0*fn2_r0*(X(3*(J1-1)+2))/r0**3

       Ez0=q*fn2_rh1*(X(3*(J1-1)+3)-r_dih(3,1))/rh1**3
       Ez0=Ez0+q*fn2_rh2*(X(3*(J1-1)+3)-r_dih(3,2))/rh2**3
       Ez0=Ez0-q0*fn2_r0*(X(3*(J1-1)+3))/r0**3

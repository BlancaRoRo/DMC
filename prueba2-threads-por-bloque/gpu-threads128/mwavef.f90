...
contains
 ...
 subroutine hpsi(w1)
  use mtiempos, only: i8t, tiempos_tic, tiempos_toc, t_valibre, t_derananum, t_vpot
  ...
  if(libre) then
   ...
   call valibre(w1)
   t_valibre=t_valibre+tiempos_toc(tt0)
  else
    call tiempos_tic(tt0)
    call derananum(w1)
    t_derananum=t_derananum+tiempos_toc(tt0)
    call tiempos_tic(tt0)
    call vpot(w1)
    t_vpot=t_vpot+tiempos_toc(tt0)
...
!  rcm=0.0_r8
 ...
 subroutine vpot(w1)
  use mtiempos, only: i8t, tiempos_tic, tiempos_toc, t_ccuerpo, t_potenbh
  type(walker), intent (inout) :: w1
  type(vec3) :: rtemp
  ...
  if(opot.eq.4) then
      call tiempos_tic(tt0)
      call ccuerpo(w1,rhe)
      t_ccuerpo=t_ccuerpo+tiempos_toc(tt0)
      call tiempos_tic(tt0)
      call potenbh(ngatom,rhe,vpotbh)
      t_potenbh=t_potenbh+tiempos_toc(tt0)
      w1%lw%pot=vpotbh
   return
  endif

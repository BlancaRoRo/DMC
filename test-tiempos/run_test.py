#!/usr/bin/env python3
"""Genera un in.mcv a partir de la plantilla, corre qmccluster_tiempos,
...
def corre(tag):
    ...
    with open(os.path.join(HIB, "in.mcv")) as fin, \
        ...
        subprocess.run(["./qmccluster_tiempos"], cwd=HIB, stdin=fin, stdout=fout, stderr=subprocess.STDOUT, check=True)
    dt = time.time() - t0
    for f in ("tiempos_opcion4.dat", "tiempos_opcion5.dat"):
        p = os.path.join(HIB, f)
        if os.path.exists(p):

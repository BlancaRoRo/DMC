...
! ---- opcion=5 (GPU, pipeline de 7 fases): puente AoS<->SoA, mas las 3
! rutinas de estadisticas por paso (dmcsumapaso, denssumapaso,
! difussumapaso), que viven fuera de pasodmc_gpu_pipeline, en el bucle
! de mmontecarlo.f90. Ver v3-cuda-optimizacion/fase3-arquitecturas-
! alternativas: el 39.7% de tiempo "invisible para CUDA" medido con
! nsys hay que repartirlo entre estas piezas para saber cual pesa de
! verdad. ----
...
contains
 ...
 subroutine tiempos_escribe7(fichero)
  ...
  real(kind=r8t) :: conversion, stats, otros
   ...
   open(97,file=fichero,status='replace')
   write(97,'(a)') 'reparto de tiempos, opcion=5 (GPU, pipeline de 7 fases)'

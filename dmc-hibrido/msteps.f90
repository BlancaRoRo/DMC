module msteps
 ...
 use mrotaciones
 use mmcvpromedia
 ! pasodmc_gpu_pipeline (opcion=5, Opcion 1 de
 ! arquitectura-streams-kin-pot.md Parte 12-13): reparto de dmc2 en 7
 ! fases + CUDA Graph en horquilla para derananum/vpot, con
 ! k_split_seeds para repartir una semilla por walker (mismo mecanismo
 ! que mrandom.md) -- pasodmc original (arriba) no se toca.
 use mrandgpu, only: k_split_seeds, rand1p_gpu
 use msync_gpu, only: sincroniza_constantes_gpu, sincroniza_etrial_gpu
 use cudafor
...
! ---- pasodmc_gpu_pipeline (opcion=5): mismo algoritmo exacto que
! pasodmc (empaqueta wsim -> SoA, corre en GPU, desempaqueta, reparte
! poblacion segun nsons -- identico), pero el bucle secuencial
! "do iwalker=1,nwpaso; call dmc2(wsim(iwalker),nsons)" se sustituye
! por el grafo de 7 fases de mdmc2_pipeline (inicializa_pipeline/
! lanza_pipeline), con derananum/vpot corriendo en paralelo dentro de
! esas fases (arquitectura-streams-kin-pot.md Parte 12-13). Los arrays
! SoA device son los persistentes de mdmc2_pipeline, dimensionados a

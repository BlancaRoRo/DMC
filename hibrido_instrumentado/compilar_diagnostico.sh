#!/bin/bash
# Compila driver_diagnostico (prueba forense final, ver comparacion-final.md):
...
# sustituyendo qmccluster.f90 por driver_diagnostico.f90 como programa
...
modlegendre.f pw_heocs.f kpcoef.f driver_diagnostico.f90"

nvfortran -cuda -Kieee -Mnofma -gpu=lineinfo -c $FILES
nvfortran -cuda -Kieee -Mnofma -gpu=lineinfo -o driver_diagnostico $(echo $FILES | sed -E 's/\.(f90|f|cuf)\b/.o/g') -llapack -lblas
echo "OK: driver_diagnostico"
